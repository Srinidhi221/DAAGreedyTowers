...
public class StrategyDP {
    ...
    private double dpValue(int[][] grid, long key, int depth) {
        if (depth >= 3)
            return 0; // horizon limit – keeps it snappy
    ...
    private double immediateReward(int[][] grid, int row, int col, int value) {
        ...
        if (opts <= 1)
            score += LOW_OPTIONS_PENALTY * 2;
        else if (opts <= 2)
            score += LOW_OPTIONS_PENALTY;
    ...
    private boolean rowVisibilityValid(int[][] grid, int row) {
        ...
        if (leftClues[row] != 0 && fromLeft != leftClues[row])
            return false;
        if (rightClues[row] != 0 && fromRight != rightClues[row])
            return false;
        return true;
    }
    ...
    private boolean colVisibilityValid(int[][] grid, int col) {
        ...
        if (topClues[col] != 0 && fromTop != topClues[col])
            return false;
        if (bottomClues[col] != 0 && fromBottom != bottomClues[col])
            return false;
        return true;
    }
