...
class AdjacencyListGraph<T> {
    ...
    Vertex<T> addVertex(T e) {
        Vertex<T> v = new Vertex<>(e);
        vertices.add(v);
        return v;
    }
...
class GameState {
    ...
    GameState(int r, int c, int v, double e) {
        this.r = r;
        this.c = c;
        this.v = v;
        this.eval = e;
...
public class TowersGamesGUI extends JFrame {
...
//    }
    ...
    private Move getBest() {
        ...
        AdjacencyListGraph<GameState> graph = new AdjacencyListGraph<>();
        Vertex<GameState> root = graph.addVertex(null); // current state

        // Build adjacency list (one layer)
        for (int r = 0; r < N; r++) {
            for (int c = 0; c < N; c++) {
                ...
                for (int v = 1; v <= N; v++) {
                    if (canPlace(r, c, v)) {
                        double e = eval(r, c, v);
                        GameState gs = new GameState(r, c, v, e);
                        Vertex<GameState> child = graph.addVertex(gs);
        ...
        // Traverse adjacency list to pick best move
        GameState best = null;
        for (Edge<GameState> edge : root.edges) {
            GameState g = edge.to.element;
            if (best == null || g.eval > best.eval)
                best = g;
        }

        if (best == null) return null;
        return new Move(best.r, best.c, best.v, best.eval);
    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new TowersGamesGUI().setVisible(true));
    }
}
